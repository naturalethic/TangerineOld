...
export function RoundedTabs({
	...
	const tabs = (Array.isArray(children) ? children : [children]).map((tab) => (
		<div
			key={tab.props.value}
			className={`px-1 cursor-pointer ${
				tab.props.value === value && "bg-zinc-500 text-zinc-100 rounded"
			}`}
...
export function FullTabs({
	...
	const onClick: MouseEventHandler = (event) => {
		const el = (event.target as HTMLElement).closest(".group") as HTMLElement;
