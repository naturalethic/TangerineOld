...
export default function DatabaseRoute() {
	...
	useEffect(() => {
		...
		Object.values(tabs).forEach((tab) => {
			if (tab.type === "query") {
				counter = Math.max(counter, Number(tab.value.split("-")[1]) + 1);
			}
		});
					...
					{tables.map((table) => (
						...
						>
							<a onClick={() => onTableMenuClick(table)}>
								{!table.startsWith("_") && <span className="invisible">_</span>}
								{table}
							</a>
						</div>
					))}
				...
				<div className="flex flex-row items-center">
					...
					>
						{Object.values(contentTabs).map((tab) => (
							<Item key={tab.value} label={tab.label} value={tab.value} />
						))}
					</FullTabs>
				</div>
